										...
										<c:if test="${fn:length(dataList) gt 0}">
											<c:forEach items="${dataList}" var="item" varStatus="status">
												<tr id="row${item.id}">
													...
													<td>${item.name}</td>
													<td class="${item.amount gt 0 ? 'red' : 'green'}">
														${item.amount > 0 ? "追投：" : "提现："}<fmt:formatNumber value="${item.amountAbs}" pattern="#0.0#"/>&nbsp;
													</td>
													<td><fmt:formatDate value="${item.deleteTime}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
													<td>
														<a href="#" class="btn mini red" onclick="deleteById(${item.id},'deleteRecordById');"><i class="icon-trash"></i>彻底删除</a>
													</td>
												</tr>
											</c:forEach>
										</c:if>
